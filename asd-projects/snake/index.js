...
function moveSnake() {
  ...
  for (i = snake.body.length - 1; i > 0; i = i - 1) {
...
function hasHitWall() {
  ...
  if (snake.head.row > ROWS - 0) {
    return true;
  }
  if (snake.head.row < 0) {
    return true;
  }
  if (snake.head.column > COLUMNS - 0) {
    return true;
  }
...
function hasCollidedWithSnake() {
  for (let i = 1; i < snake.body.length; i = i + 1)
    if (
      snake.head.row === snake.body[i].row - 0 &&
      snake.head.column === snake.body[i].column - 0
    ) {
      return true;
    }
  /* 
...
function getRandomAvailablePosition() {
  var appleIsInsideSnake = true;
  var randomPosition = {};

  /* Generate random positions until one is found that doesn't overlap with the snake */
  while (appleIsInsideSnake) {
    randomPosition.column = Math.floor(Math.random() * COLUMNS);
    randomPosition.row = Math.floor(Math.random() * ROWS);
    appleIsInsideSnake = false;
    for (let i = 0; i < snake.body.length; i = i + 1) {
      if (
        randomPosition.row === snake.body[i].row - 0 &&
        randomPosition.column === snake.body[i].column - 0
      ) {
        appleIsInsideSnake = true
      }
    }
